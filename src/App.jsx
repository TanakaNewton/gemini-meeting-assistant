...
function parseTranscriptionResponse(responseText) {
  ...
  if (parsedResults.length === 0 && responseText.trim()) {
    console.warn(
      "Could not parse the response into lines. Displaying raw text."
    );
...
const availableModels = [{ id: "gemini-2.5-pro", name: "Gemini 2.5 Pro" }];
...
const markdownComponents = {
  ...
  ul: (props) => (
    <List
      dense
      sx={{ paddingLeft: 2, listStyleType: "disc", pl: 4 }}
      {...props}
    />
  ), // デフォルトの黒丸
  ol: (props) => (
    <List
      dense
      component="ol"
      sx={{ paddingLeft: 2, listStyleType: "decimal", pl: 4 }}
      {...props}
    />
  ), // 番号付きリスト
  li: (props) => (
...
function App() {
  ...
  useEffect(() => {
    const uniqueDefaultSpeakers = [
      ...new Set(results.map((r) => r.defaultSpeaker)),
    ].filter(Boolean);
  ...
  const uniqueDefaultSpeakersForBulkEdit = useMemo(() => {
    return [...new Set(results.map((r) => r.defaultSpeaker))]
      .filter(Boolean)
      .sort();
  }, [results]);
  ...
  const handleTranscribe = async () => {
    ...
    try {
      ...
      const statusCode =
        err?.code || err?.response?.status || err?.response?.error?.code;
      ...
      if (
        ...
        if (Array.isArray(errorDetails)) {
          ...
          if (quotaFailure && Array.isArray(quotaFailure.violations)) {
            const modelViolation = quotaFailure.violations.find(
              (v) => v?.quotaDimensions?.model === selectedModel
            );
            if (modelViolation) {
        ...
        if (isModelSpecificQuota) {
          const modelDisplayName =
            availableModels.find((m) => m.id === violatedModel)?.name ||
            violatedModel;
          errorMessage = `選択されたモデル (${modelDisplayName}) の無料利用枠の上限に達したか、一時的に利用が制限されています。他のモデルを試すか、時間をおいて再度お試しください。`;
        } else {
          errorMessage =
            "APIの利用上限(クォータ)に達した可能性があります。プランを確認するか、時間をおいて再度お試しください。";
        }
      } else if (
        statusCode === 400 &&
        ...
        errorMessage =
          "APIキーが無効、または選択したモデルへのアクセス権がありません。キーとモデルの組み合わせを確認してください。";
      } else if (statusCode === 403) {
        errorMessage =
          "選択したモデルへのアクセスが拒否されました。APIキーに必要な権限が付与されているか確認してください。";
      } else if (
        statusCode === 404 &&
        ...
        prevResults.map((row) => {
          if (row.id === rowId) {
            if (editingField === "speaker") {
              return {
                ...row,
                displaySpeaker: newValue.trim() || row.defaultSpeaker || "不明",
              };
  ...
  const handleDeleteRow = (idToDelete) => {
    setResults((prevResults) =>
      prevResults.filter((row) => row.id !== idToDelete)
    );
  ...
  const handleDownloadCsv = () => {
    ...
    downloadFile(
      content,
      `transcription_${getTimestamp()}.csv`,
  ...
  const handleDownloadMarkdownTranscription = () => {
    ...
    const markdownString = results
      .map((row) => `**${row.displaySpeaker || "不明"}:** ${row.text}`)
      .join("\n\n");
    downloadFile(
      markdownString,
      `transcription_${getTimestamp()}.md`,
  ...
  const handleDownloadTxtTranscription = () => {
    ...
    const txtString = results
      .map((row) => `${row.displaySpeaker || "不明"}: ${row.text}`)
      .join("\n\n");
    downloadFile(
      txtString,
      `transcription_${getTimestamp()}.txt`,
  ...
  const handleSummarize = async () => {
    ...
    try {
      const conversationText = results
        .map((row) => `${row.displaySpeaker}: ${row.text}`)
        .join("\n");
      ...
      const markdownBlockMatch = summaryText.match(
        /^```markdown\s*([\s\S]*?)\s*```$/
      );
  ...
  const handleExtractKeywords = async () => {
    if (!results || results.length === 0 || !apiKey) {
      setKeywordsError(
        "キーワードを抽出する文字起こし結果またはAPIキーがありません。"
      );
      return;
    }
...
\`\`\``;
      ...
      try {
        ...
        if (
          ...
          throw new Error(
            "API応答が期待されたJSON配列(文字列)形式ではありません。"
          );
        }
        ...
        throw new Error(
          `キーワードリストの解析に失敗しました。応答が期待されるJSON配列形式ではありませんでした。\n詳細: ${e.message}`
        );
      }
      setKeywords(items);
    } catch (err) {
  ...
  const handleExtractActionItems = async () => {
    if (!results || results.length === 0 || !apiKey) {
      setActionItemsError(
        "アクションアイテムを抽出する文字起こし結果またはAPIキーがありません。"
      );
      return;
    }
...
\`\`\``;
      ...
      try {
        ...
        if (
          !Array.isArray(items) ||
          (items.length > 0 &&
            (typeof items[0].assignee === "undefined" ||
              typeof items[0].task === "undefined"))
        ) {
            ...
            {results.length > 0 && !isLoading && (
              <>
                ...
                >
                  ...
                  <MenuItem onClick={handleDownloadMarkdownTranscription}>
                    Markdown形式 (.md)
                  </MenuItem>
                  <MenuItem onClick={handleDownloadTxtTranscription}>
                    テキスト形式 (.txt)
                  </MenuItem>
                </Menu>
              </>
        ...
        {results.length > 0 && !isLoading && (
          <>
              ...
              <Grid container spacing={2} alignItems="center">
                  ...
                  <Grid xs={12} sm={6} md={4} key={defaultSpeaker}>
                    <Box sx={{ display: "flex", alignItems: "baseline" }}>
                      ...
                      <TextField
                        ...
                        onChange={(e) =>
                          handleBulkNameChange(e, defaultSpeaker)
                        }
                        disabled={
                          isLoading ||
                          isSummarizing ||
                ...
                <Grid xs={12}>
                  <Button
                    ...
                    disabled={
                      Object.values(bulkEditNames).every(
                        (name) => !name?.trim()
                      ) ||
                      isLoading ||
                      isSummarizing ||
            ...
            >
              <Table
                sx={{ minWidth: 750 }}
                aria-label="transcription results table"
              >
                <TableHead>
                  <TableRow sx={{ position: "relative" }}>
                    ...
                    <TableCell
                      align="center"
                      sx={{ width: "60px", position: "relative" }}
                    >
                      {/* ヘッダー右下に行頭追加ボタンを配置 */}
                ...
                <TableBody>
                  {results.map((row, index) => {
                    ...
                    const isEditingThisSpeaker =
                      editingRowId === row.id && editingField === "speaker";
                    const isEditingThisText =
                      editingRowId === row.id && editingField === "text";
                      ...
                      >
                        <TableCell
                          ...
                          sx={{
                            ...
                            verticalAlign: "top",
                            width: "15%",
                            cursor: !isEditingThisSpeaker
                              ? "pointer"
                              : "default",
                            minHeight:
                              editingRowId === row.id ? "auto" : "2.5em",
                            height: "auto",
                          }}
                          onClick={() =>
                            ...
                            handleCellClick(row.id, "speaker", displayName)
                          }
                        >
                          {isEditingThisSpeaker ? (
                            <ClickAwayListener
                              onClickAway={() => handleValueSave(row.id)}
                            >
                              <TextField
                                value={editingValue}
                                onChange={(e) =>
                        ...
                        <TableCell
                          ...
                          onClick={() =>
                            ...
                            handleCellClick(row.id, "text", row.text)
                          }
                        >
                          {isEditingThisText ? (
                            <ClickAwayListener
